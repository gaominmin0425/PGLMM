para_bootstrap <- function(B, Dataset){
  ...
  mu_i_hat = exp(log(Dataset1$Area)+tortoise_fit$beta[1]+(tortoise_fit$beta[2]*Dataset1$prev
                                                         +tortoise_fit$beta[3]*factor(Dataset1$Year2005)
                                                         +tortoise_fit$beta[4]*factor(Dataset1$Year2006)
                                                         +Zi_1))
  ...
  Bootstrap1 = Dataset_new %>% group_by(B) %>% summarize(tidy(glmer(Y_hat~prev +offset(log(Area))+factor(year)+(1|site),
